...
export function runExchangeTests(
  ...
  describe('Exchange', function () {
    ...
    describe('checkOrderValidity', function () {
      let order: Order;
      ...
      it('trader is zero', async () => {
        ...
        expect(
          await exchange.checkOrderValidity(order.parameters, orderHash),
        ).to.equal(false);
      });
      ...
      it('cancelled or filled', async () => {
        await exchange.connect(alice).cancelExchangeOrder(order.parameters);
        expect(
          await exchange.checkOrderValidity(order.parameters, orderHash),
        ).to.equal(false);
      });
